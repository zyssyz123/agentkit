"""``agentkit-eval`` CLI: run a suite.yaml and print a pretty report."""
...
from agentkit_eval.harness import EvalReport, load_suite, run_suite_sync

app = typer.Typer(
    add_completion=False,
    no_args_is_help=True,
    help="Run a declarative AgentKit eval suite.",
)
console = Console()
...
def _print_report(report: EvalReport) -> None:
    table = Table(title="AgentKit eval results")
...
def _to_junit(report: EvalReport) -> str:
    ...
    for r in report.results:
        if r.passed:
            cases_xml.append(
                f'    <testcase classname="agentkit-eval" name="{escape(r.case.name)}" '
    ...
    return (
        ...
        f'<testsuite name="agentkit-eval" tests="{report.total}" '
