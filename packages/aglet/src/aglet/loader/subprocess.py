...
Bidirectional newline-delimited JSON. Client (Aglet Runtime) initiates calls;
...
Plugins receive lossy dict snapshots of AgentContext (see ``aglet.serialize``).
This is good enough for common Tool / Memory / Safety techniques and avoids forcing
plugins to import the full Aglet Python types — which keeps subprocess plugins
...
from aglet.context import AgentContext, ContextPatch, MemoryItem, ToolCall, ToolResult, ToolSpec
from aglet.protocols import HealthStatus
from aglet.serialize import (
    context_to_dict,
    memory_item_to_dict,
    patch_from_dict,
...
async def load_subprocess_plugin(
    ...
    register them with the global :class:`aglet.registry.Registry`.
    """
