...
1. **Python entry points** (preferred for distribution): a plugin's ``pyproject.toml`` exposes
   ``[project.entry-points."agentkit.techniques"]`` mapping ``<element>.<name>`` to a
...
ENTRY_POINT_GROUP_TECHNIQUES = "agentkit.techniques"
ENTRY_POINT_GROUP_ELEMENTS = "agentkit.elements"
