"""observability.otel — emit Aglet Events as OpenTelemetry spans.
...
class OtelObservability:
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        cfg = config or {}
        self.service_name: str = cfg.get("service_name", "aglet")
        ...
        try:
            ...
            pass
        self._tracer = trace.get_tracer("aglet", "0.1.0")
    ...
    async def on_event(self, event: Event) -> None:
        ...
        if event.type == EventType.RUN_STARTED:
            span = self._tracer.start_span(name="aglet.run", attributes={"run.id": run_id})
