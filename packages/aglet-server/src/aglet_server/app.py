"""FastAPI application that exposes registered Aglet agents via REST + SSE.
...
from aglet.config import AgentConfig, load_agent_config
...
def build_app(agents: list[Path] | None = None) -> FastAPI:
    ...
    app = FastAPI(
        title="Aglet Server",
        version="0.1.0",
        description="HTTP + SSE interface for the Aglet runtime.",
    )
